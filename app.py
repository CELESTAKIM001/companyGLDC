...
APP_URL = str(env('APP_URL', default='https://companygldc.vercel.app')).strip().rstrip('/')
# APP_URL is the single source of truth for every public URL generated by GLDC.
# Keep the production fallback canonical so certificates, QR codes, receipts,
# emails and payment links cannot silently point to an old deployment.
if not APP_URL:
    APP_URL = 'https://companygldc.vercel.app'

def public_url(path=''):
    path='/' + str(path or '').lstrip('/')
    return f'{APP_URL}{path}'

def certificate_verification_url(certificate_no):
    return public_url(f'/membership/certificate/{certificate_no}')

def receipt_verification_url(receipt_code):
    return public_url(f'/receipt/{receipt_code}')

def invoice_verification_url(invoice_number):
    return public_url(f'/invoice/{invoice_number}')
...
def membership_certificate_verify_page(certificate_no):
    ...
    c=clean_doc(c); c['verificationUrl']=certificate_verification_url(certificate_no); return render_template('membership_certificate_verify.html',title='Certificate Verification',certificate=c)
...
def membership_certificate_verify_api(certificate_no):
    ...
    c=clean_doc(c); c['verificationUrl']=certificate_verification_url(certificate_no); return jsonify(ok=True,certificate=c)
...
def _ensure_certificate_pdf(certificate, member):
    """Return the official certificate PDF, rebuilding/re-archiving it when Drive is missing."""
    plan=_membership_plan(certificate.get('planId','')) or {'id':certificate.get('planId'),'name':certificate.get('planName','GLDC Membership'),'months':1}
    drive_id=certificate.get('driveFileId')
    if drive_id:
        try:
            data=drive_download_bytes_by_id(drive_id)
            if bytes(data).startswith(b'%PDF-'):
                return data
        except Exception:
            app.logger.exception('Certificate Drive read failed cert=%s; rebuilding',certificate.get('certificateNumber'))
    canonical_verify=certificate_verification_url(certificate.get('certificateNumber'))
    if certificate.get('verificationUrl') != canonical_verify:
        try:
            db.membership_certificates.update_one({'_id':certificate['_id']},{'$set':{'verificationUrl':canonical_verify,'updatedAt':now()}})
            certificate['verificationUrl']=canonical_verify
        except Exception:
            app.logger.exception('Unable to canonicalize certificate verification URL cert=%s',certificate.get('certificateNumber'))
    data=build_membership_certificate(member,plan,certificate.get('certificateNumber'),certificate.get('validFrom'),certificate.get('validUntil'),certificate.get('issuedAt'))
    # Drive is an archive, not the only source of truth. If Drive is temporarily
    # unavailable, the member still receives the valid database-backed certificate PDF.
    try:
        drive=drive_upload_bytes(f"{certificate.get('certificateNumber')}.pdf",data,'application/pdf')
        did=drive.get('id')
        if did:
            db.membership_certificates.update_one({'_id':certificate['_id']},{'$set':{'driveFileId':did,'updatedAt':now()}})
            db.members.update_one({'id':member.get('id')},{'$set':{'certificateDriveId':did,'updatedAt':now()}})
    except Exception:
        app.logger.exception('Certificate Drive archive unavailable cert=%s; serving regenerated PDF',certificate.get('certificateNumber'))
    return data
...
def member_certificate_history_download(certificate_no):
    ...
    if not c: return json_error('Certificate not found.',404,'CERTIFICATE_NOT_FOUND')
    try:
        data=_ensure_certificate_pdf(c,m)
        return Response(data,mimetype='application/pdf',headers={'Content-Disposition':f'inline; filename="GLDC-{certificate_no}.pdf"','Content-Length':str(len(data))})
    except Exception: return json_error('Unable to generate or retrieve the certificate.',503,'CERTIFICATE_DOWNLOAD_FAILED')
...
def member_certificate_download():
    ...
    if not m: return json_error('Please sign in to your member portal.',401,'AUTH_REQUIRED')
    cert=None
    if m.get('certificateNumber'):
        cert=db.membership_certificates.find_one({'certificateNumber':m.get('certificateNumber'),'memberId':m.get('id')})
    if not cert:
        cert=db.membership_certificates.find_one({'memberId':m.get('id'),'status':'ACTIVE'},sort=[('issuedAt',DESCENDING)])
    if not cert: return json_error('Your certificate has not been generated yet. GLDC approval is required before a certificate can be issued.',404,'CERTIFICATE_NOT_AVAILABLE')
    try:
        data=_ensure_certificate_pdf(cert,m)
        return Response(data,mimetype='application/pdf',headers={'Content-Disposition':f'inline; filename="GLDC-{cert.get("certificateNumber","membership-certificate")}.pdf"','Content-Length':str(len(data))})
    except Exception: return json_error('Unable to generate or retrieve your membership certificate.',503,'CERTIFICATE_DOWNLOAD_FAILED')
...
def _certificate_email_payload(cert, member, plan):
    ...
    verify_url=certificate_verification_url(cert_no)
    pdf=None
...
@app.post('/api/admin/membership/certificates/<certificate_no>/generate')
@admin_required
def admin_generate_membership_certificate(certificate_no):
    cert=db.membership_certificates.find_one({'certificateNumber':certificate_no})
    if not cert: return json_error('Certificate record not found.',404,'CERTIFICATE_NOT_FOUND')
    member=db.members.find_one({'id':cert.get('memberId')})
    if not member: return json_error('Member record not found.',404,'MEMBER_NOT_FOUND')
    try:
        data=_ensure_certificate_pdf(cert,member)
        audit('MEMBERSHIP_CERTIFICATE_GENERATED','certificate',certificate_no,{'memberId':member.get('id'),'admin':current_user().get('email')})
        return jsonify(ok=True,message='Certificate PDF generated and archived when Google Drive was available.',certificateNumber=certificate_no,verificationUrl=certificate_verification_url(certificate_no))
    except Exception as exc:
        return json_error(str(exc),503,'CERTIFICATE_GENERATION_FAILED')
...
def admin_receipt_download(receipt_code):
    ...
    p['verificationUrl']=receipt_verification_url(receipt_code)
...
def admin_create_membership_receipt():
    try:
        ...
        if b.get('sendEmail',False) and m and m.get('email'):
            send_email(m['email'],f'GLDC Payment Receipt {code}',f'Your GLDC payment receipt is {receipt_verification_url(code)}.',f'<p>Your GLDC payment receipt is <b>{code}</b>.</p><p><a href="{receipt_verification_url(code)}">VERIFY RECEIPT</a></p>',[(code+'.pdf',pdf,'application/pdf')])
        return jsonify(ok=True,receipt={'receiptCode':code,'verificationUrl':receipt_verification_url(code),'downloadUrl':f'/api/admin/membership/receipts/{code}/download'})
    except Exception:
...
def invoice_verify_page(invoice_number):
    ...
    inv=clean_doc(inv); inv['verificationUrl']=invoice_verification_url(invoice_number)
    return render_template('invoice_verify.html',title='Invoice Verification',invoice=inv)
